...
def GetAOIItems(modnum,myrunid,modnum2,myrunid2):
	...
	try:
		...
		waferlist = ['N46769.1-12A6']

		aoicol = mydb["PICDIEAOI"]
		query = {'$and':[{'Project':'800G_DR8_SiPh'},{'Wafer':{'$in':waferlist}},{'Analyzed':{'$in':[0,3]}}]}
...
def MainLoop():
	...
	print('python ORION_AOIx.py  --gpuid  '+args.gpuid+'  --modnum  '+str(args.modnum)+'  --runid  '+str(args.runid)+'  --modnum2  '+str(args.modnum2)+'  --runid2  '+str(args.runid2))
				...
				for param in chunck:
					...
					if len(AOIRESTList) == 10:
						...
						if solved%200 == 0:
							...
							if timespan > 1800:
								print("WARNING: the network seems have problem, it spend "+str(timespan)+" seconds to complete 200 tasks")
							prevtime = datetime.now()
